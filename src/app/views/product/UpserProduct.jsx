...
function CustomTabPanelVariations(props) {
  ...
  const fields = [
    ...
    {
      key: "availStock",
      id: "availableStockAmount",
      type: "number",
      label: "Available stock amount",
      placeholder: "Enter available stock amont",
      value: "",
    },
    {
      key: "penStock",
      id: "pendingStockAmount",
      type: "number",
      label: "Pending stock amount",
      placeholder: "Enter pending stock amont",
      value: "",
    },
    {
      key: "orderType",
      id: "backendOrderType",
      type: "radio",
      label: "Backend order type",
      value: "ALLOWED_ALL",
      options: [
        { value: "ALLOWED_ALL", label: "Allowed" },
        { value: "ALLOWED_ADMIN", label: "Admin only" },
  ...
  const {
    ...
    updateVariations,
    setUpdateVariations,
    updateVariableAttributeList,
    attributes,
    variables,
    setVariables,
    variationIdentifier,
    setVariationIdentifier,
  ...
  const checkFields = (field, current) => {
    return requiredFields
      ...
      ?.reduce((acc, cur) => {
        return (
          (cur === "variationAttributes"
            ? current[cur].length !== 0 &&
              current[cur].filter(
                (itm) => itm.attributeVariableId === undefined
              ).length === 0
            : cur === "variationImage" || cur === "backendOrderType"
            ? current[cur] !== undefined && current[cur] !== ""
            : current[cur] !== undefined &&
              current[cur] !== "" &&
              parseFloat(current[cur]) > 0) && acc
        );
      }, true);
  };
      ...
      ?.reduce((acc, cur) => {
        return (
          (current.val === undefined ||
            current.val === "" ||
            (/^.*\..*$/.test(current[cur])
              ? parseFloat(current[cur])
              : parseInt(current[cur])) >= 0) &&
          acc
        );
      }, true);
  };
  ...
  const generateVariations = (list) => {
    ...
    const val =
      list.length === 0
        ...
        : list.filter(
            (val) =>
              val.state === "update" && val.attributeVariables.length !== 0
          );
    const sets =
      val !== undefined && Array.isArray(val) && val.length !== 0
        ? val.map((itm) =>
            itm.attributeVariables.map((lst) => ({
              attributeId: itm.attributeId,
              attributeVariableId: lst.variableId,
            }))
          )
        : [];
  ...
  const addVariationManual = () => {
    if (variableAttributeList.length > 0) {
      ...
      newList.unshift({
        ...
        variationIdentifier: newIdentifier,
        state: "create",
        variationAttributes: variableAttributeList
          .filter(
            (itm) => itm.state === "update" && itm.attributeVariables.length > 0
          )
          ?.map((itm) => ({
            attributeId: itm.attributeId,
            attributeVariableId: undefined,
          })),
        unitCost: 0,
        unitPrice: 0,
        availableStockAmount: 0,
  ...
  const upsertVariations = async () => {
    const handleSuccessfull = async (results, messages) => {
      ...
      results.forEach((result) => {
        const variation = newList.find(
          (itm) => itm.variationIdentifier === result.variationIdentifier
        );
        if (variation) {
          messages.push({
            text: `Variation with variation id ${result.variationId} is ${
              variation.state === "create" ? "created" : "update"
            }.`,
            variant: "success",
          });
    ...
    const handleErrors = async (errors, messages) => {
      ...
      Object.keys(errors).forEach((key) => {
        ...
        if (typeof error === "string")
          messages.push({ text: error, variant: "error" });
        if (typeof error === "object") {
          const { variationAttributes, variationImage, ...rest } = error;
          if (variationAttributes !== undefined)
            messages.push({ text: variationAttributes, variant: "error" });
          if (variationImage !== undefined)
            messages.push({ text: variationImage, variant: "error" });
          if (rest !== undefined) {
            messages.push({
              text: `Invalid inputs entered in variation #${key}.`,
              variant: "error",
            });
            newObj[key] = rest;
          }
    ...
    const convertVariations = async (variations, state, excludeKeys) => {
      return Promise.all(
        variations
          ...
          .map(async (lst) => {
            try {
              const result = await Object.keys(lst).reduce(
      ...
      const requests = [
        {
          ...
          method: "post",
          data: {
            variationUpsertDtos: await convertVariations(
              updateVariations,
              "create",
              ["state", "expanded", "checked", "variationId"]
            ),
          },
          ...
          error: async (error) => {
            if (
              ...
              if (typeof error.response.data === "string")
                messages.push({ text: error.response.data, variant: "error" });
              if (
              ...
              if (
                ...
                const newList = [...variationIds];
                error.response.data.result.forEach((itm) =>
                  newList.push(itm.variationId)
                );
                setVariationIds(newList);
              }
            ...
            if (error.response.status === 500)
              messages.push({
                text: "Error occured during creating variations.",
                variant: "error",
              });
          },
        },
        {
          ...
          error: async (error) => {
            ...
            if (error.response.status === 500)
              messages.push({
                text: "Error occured during updating variations.",
                variant: "error",
              });
          },
        },
          ...
          requests
            .map(async (request) => {
              if (
                ...
                request.data.variationUpsertDtos.length > 0
              ) {
                return api({
  ...
  useEffect(() => {
    if (changedIdentifier.length > 0) {
      ...
      variations
        .filter((lst) => changedIdentifier.includes(lst.variationIdentifier))
        ?.forEach((lst) => {
          if (checkFields("", lst) && checkNonRequiredFields("", lst)) {
            const updatedFields =
              bulkOption === "all"
                ...
                : fields
                    .filter((itm) => itm.key === bulkOption)
                    .map((field) => field.id);

            if (
              updatedList.length === 0 ||
              updatedList.filter(
                (itm) => itm.variationIdentifier === lst.variationIdentifier
              ).length === 0
            ) {
              lst.state === "create"
                ? updatedList.push(lst)
                : updatedList.push({
                    attributeId: lst.attributeId,
                    variationIdentifier: lst.variationIdentifier,
                    state: lst.state,
                    attributeVariables: lst.attributeVariables,
                    ...updatedFields.reduce(
                      (acc, key) => (acc[key] = lst[key]),
                      {}
                    ),
                  });
            } else {
              const listItm = updatedList.find(
                (itm) => itm.variationIdentifier === lst.variationIdentifier
              );
      ...
      {value === index && (
        <Box sx={{ p: 3 }}>
          <Stack spacing={2}>
            ...
            (() => {
              const val = variableAttributeList.filter(
                (val) =>
                  val.state === "update" && val.attributeVariables.length !== 0
              );
              return (
              ...
              <React.Fragment>
                <Typography>
                  Generate all possible variations or add variations manually.
                </Typography>
                <Grid
                  sx={{
                    display: "flex",
                ...
                >
                  ...
                  <FormControl
                    sx={{ width: "220px" }}
                    disabled={
                      variations.filter((val) => val.checked).length === 0
                    }
                  >
                ...
                />
                <CustomVariationExpansionPanel
                  checkNonRequiredFields={checkNonRequiredFields}
                  checkFields={checkFields}
                  setChangedIdentifier={setChangedIdentifier}
                  variationErrors={variationErrors}
                  variableAttributeList={variableAttributeList}
                  updateVariations={updateVariations}
                  setUpdateVariations={setUpdateVariations}
              ...
              >
                <Icon
                  sx={{ fontSize: "5.5em", color: "gray", fontWeight: "small" }}
                >
                  info_outline
                </Icon>
                <Typography>{`${
                  updateVariableAttributeList.length !== 0
                    ? "Save unsaved attribute changes"
                    : "Add some attributes"
                } in the Attributes tab to generate variations.`}</Typography>
              </Grid>
            )}
          </Stack>
        </Box>
...
function CustomTabPanelAttributes(props) {
  ...
  const addExistingAttribute = async () => {
    ...
    let attribute =
      variables.length !== 0
        ? variables.find((itm) => itm.attributeId === selectedAttributes.value)
        : undefined;

    if (!attribute) {
      setAddAttributeLoading(true);
      await api
        .get(`/product/attribute/get/${selectedAttributes.value}`)
        .then((res) => {
          if (res.status === 200) {
            const newList = [...variables];
            attribute = {
              attributeId: res.data.attributeId,
              attributeName: res.data.attributeName,
              variableList: res.data.attributeVariables.map((itm) => ({
                variableId: itm.attributeVariableId,
                variableName: itm.attributeVariableName,
              })),
            };
  ...
  const upsertAttributes = async () => {
    const handleSuccessfull = (results, messages) => {
      ...
      results.forEach((result) => {
        const attribute = newList.find(
          (itm) => itm.identifier === parseInt(result.identifier)
        );
        if (attribute) {
          messages.push({
            text: `${attribute.attributeName} attribute is created.`,
            variant: "success",
          });
          attribute.attributeId = result.attributeId;
          attribute.attributeVariables = result.attributeVariables.map(
            (itm) => {
              messages.push({
                text: `${itm.attributeVariableName} attribute variable is added to the ${attribute.attributeName} attribute.`,
                variant: "success",
              });
              return {
                variableId: itm.attributeVariableId,
        ...
        newVar.push({
          ...
          variableList: result.attributeVariables.map((itm) => ({
            variableId: itm.attributeVariableId,
            variableName: itm.attributeVariableName,
          })),
        });
    ...
    if (updateVariableAttributeList.length !== 0) {
      const newList = updateVariableAttributeList
        ...
        ?.map((itm) => ({
          attribute: itm.attributeName,
          identifier: itm.identifier,
          variables: itm.attributeVariables,
        }));
      ...
      newList.length !== 0 &&
        (await api
          .post("/product/attribute/create", newList)
          .then((res) => {
            if (res.status === 201) {
          ...
          .catch((err) => {
            if (err.response.status === 400 || err.response.status === 409) {
              if (typeof err.response.data === "string")
                messages.push({ text: err.response.data, variant: "error" });
              if (
                ...
                Object.keys(errors).forEach((key) => {
                  if (errors[key].attribute !== undefined)
                    messages.push({
                      text: errors[key].attribute[0],
                      variant: "error",
                    });
                  if (errors[key].variables !== undefined)
                    errors[key].variables.forEach((val) =>
                      messages.push({ text: val, variant: "error" })
                    );
                });
              }
              if (
      ...
      {value === index && (
        <Box sx={{ p: 3 }}>
          <Stack spacing={2}>
            <Stack spacing={2}>
              <Typography>
                Create new attribute with variables for the product or find
                existing attribute and customize existing variables in it for
                the product.
              </Typography>
              <Grid
                sx={{
              ...
              >
                <SearchableSelectMultiple
                  ...
                  sx={{ width: "20%", maxWidth: "200px", minWidth: "150px" }}
                ></SearchableSelectMultiple>
                ...
                <LoadingButton
                  ...
                  color="primary"
                  startIcon={
                    addAttributeLoading ? <CircularProgress size={20} /> : null
                  }
                  disabled={
                ...
                <Button
                  variant="contained"
                  color="primary"
                  onClick={addNewVariableAttribute}
                >
            ...
            <CustomExpansionPanel
              ...
              setUpdateVariableAttributeList={setUpdateVariableAttributeList}
              removeFromVariableAttributeList={removeFromVariableAttributeList}
              variables={variables}
              editVariableAttributeList={editVariableAttributeList}
              list={variableAttributeList}
              isDeleteOn={true}
            ></CustomExpansionPanel>
...
function UpsertProduct({ update, id }) {
  ...
  const [updateVariableAttributeList, setUpdateVariableAttributeList] =
    useState([]);
  ...
  useEffect(() => {
    const getAttributes = async () => {
      await api
        .get("/product/attribute/view")
        .then((res) => {
          if (res.status === 200)
            setAttributes(
              res.data.map((itm) => ({
                label: itm.attributeName,
                value: itm.attributeId,
              }))
            );
        })
        .catch((err) => {});
          ...
          if (res.status === 200)
            setCategories(
              res.data.map((itm) => ({
                ...
                subCategories: itm.subCategories.map((val) => ({
  ...
  const upsertProduct = async () => {
    ...
    if (Object.keys(productErrors).length !== 0) {
      Object.keys(productErrors).forEach((key) => {
        errors.push({ text: productErrors[key], variant: "error" });
      });
    }
    ...
    productImage !== "" &&
      (await api
        .post(`/product/${update ? `update/${id}` : "create"}`, formData, {
          headers: {
  ...
  const nameValidation = (
    ...
    if (
      ...
      newObj[
        field
      ] = `${fieldLabel} must be in between ${min} and ${max} characters.`;
      setProductErrors(newObj);
      return false;
    }
    delete newObj[field];
    ...
    <Container>
      ...
      <Stack spacing={3}>
              ...
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (
                    ...
                    newObj.productTitle = e.target.value;
              ...
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (
                    ...
                    newObj.productSubTitle = e.target.value;
              ...
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (
                    ...
                    newObj.productShortDescription = e.target.value;
                              ...
                              >
                                <SmallCheckbox
                                  ...
                                  onClick={(event) => {
                                    ...
                                    const categoryIndex = newList.findIndex(
                                      (cat) => cat.categoryId === category.id
                                    );
                                    if (categoryIndex === -1)
                                      newList.push({
                                      ...
                                      const subCategoryIndex = newList[
                                        categoryIndex
                                      ...
                                      if (subCategoryIndex === -1)
                                        ...
                                        if (
                                          newList[categoryIndex].subCategories
                                            .length === 0
                                        )
        ...
        >
          ...
          <Grid
            sx={{ maxHeight: "600px", overflowY: "scroll", marginTop: "1em" }}
          >
            <CustomTabPanelAttributes
              ...
              setVariations={setVariations}
              setAttributeIdentifier={setAttributeIdentifier}
              attributeIdentifier={attributeIdentifier}
              setVariableAttributeList={setVariableAttributeList}
              value={value}
              index={0}
              attributes={attributes}
              variableAttributeList={variableAttributeList}
            ...
            <CustomTabPanelVariations
              ...
              setVariationIdentifier={setVariationIdentifier}
              variableAttributeList={variableAttributeList}
              variations={variations}
              setVariations={setVariations}
              value={value}
        ...
        >
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            startIcon={<Icon sx={{ fontSize: "0.75em" }}>publish</Icon>}
            onClick={upsertProduct}
            disabled={
              loading ||
              updatedProductDetails === null ||
              Object.keys(updatedProductDetails).length === 0 ||
